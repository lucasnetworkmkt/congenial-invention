...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ reservations, menuItems, onUpdateStatus, onUpdateMenuPrice, onLogout, isDbConnected }) => {
  ...
  // Ensure we never work with null/undefined arrays
  ...
  // SQL Command Generation - Simplified to prevent crashes
        ...
        {activeTab === 'menu' && (
          <div className="bg-stone-900 rounded-2xl border border-stone-800 p-6">
             <div className="mb-4 flex justify-between items-center">
               <h3 className="font-bold text-white">Preços do Cardápio</h3>
               <button onClick={handleResetMenu} className="text-xs text-red-400 underline">Resetar Tabela</button>
             </div>
             
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {cleanMenuItems.map((item) => (
                  <div key={item.id} className="flex items-center justify-between bg-stone-950 p-3 rounded border border-stone-800">
                    <div className="flex items-center gap-3">
                      {item.image && <img src={item.image} className="w-10 h-10 rounded object-cover" alt="" />}
                      <div>
                    ...
                    {editingPriceId === item.id ? (
                       ...
                       <div className="flex items-center gap-2">
                         ...
                         <button onClick={() => startEditingPrice(item)} className="p-1 text-stone-500 hover:text-white"><Edit2 size={14}/></button>
                       </div>
                    )}
                  </div>
                ))}
                {cleanMenuItems.length === 0 && <p className="text-stone-500">Sem itens. Tente resetar.</p>}
             </div>
          </div>
        )}
